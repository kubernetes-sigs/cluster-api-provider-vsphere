...
//+kubebuilder:webhook:verbs=create;update,path=/validate-infrastructure-cluster-x-k8s-io-v1alpha3-vspherefailuredomain,mutating=false,failurePolicy=fail,matchPolicy=Equivalent,groups=infrastructure.cluster.x-k8s.io,resources=vspherefailuredomains,versions=v1alpha3,name=validation.vspherefailuredomain.infrastructure.cluster.x-k8s.io,sideEffects=None
//+kubebuilder:webhook:path=/mutate-infrastructure-cluster-x-k8s-io-v1alpha3-vspherefailuredomain,mutating=true,failurePolicy=fail,matchPolicy=Equivalent,groups=infrastructure.cluster.x-k8s.io,resources=vspherefailuredomains,verbs=create;update,versions=v1alpha3,name=default.vspherefailuredomain.infrastructure.cluster.x-k8s.io,sideEffects=None
...
func (r *VSphereFailureDomain) Default() {
	if r.Spec.Zone.AutoConfigure == nil {
		r.Spec.Zone.AutoConfigure = pointer.BoolPtr(false)
	}

	if r.Spec.Region.AutoConfigure == nil {
		r.Spec.Region.AutoConfigure = pointer.BoolPtr(false)
	}
}
