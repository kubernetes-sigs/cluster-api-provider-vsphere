...
func (r clusterIdentityReconciler) Reconcile(req ctrl.Request) (_ reconcile.Result, reterr error) {
	...
	defer func() {
		...
		if err := patchHelper.Patch(r, identity); err != nil {
			if reterr == nil {
				reterr = err
			}
	...
	if !identity.DeletionTimestamp.IsZero() {
		return r.reconcileDelete(r, identity)
	}
