...
func (r *machineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	// machine context with provider-specific configuration. When the ClusterNetworkProvider feature
	// gate is enabled the provider is resolved from VSphereCluster.spec.network.provider; otherwise
	// the static provider built from the --network-provider flag is returned.
	var np services.NetworkProvider
	if r.supervisorBased {
		supervisorMachineCtx, ok := machineContext.(*vmware.SupervisorMachineContext)
		if !ok {
			return reconcile.Result{}, errors.New("received unexpected SupervisorMachineContext type")
		}
		...
		if err != nil {
			...
			return reconcile.Result{}, errors.Wrapf(err, "failed to resolve network provider")
		}
