...
func (r vmReconciler) reconcileIPAddressClaims(ctx context.Context, vmCtx *capvcontext.VMContext) error {
	...
	var (
		claims        []deprecatedconditions.Getter
		v1beta2Claims []deprecatedv1beta2conditions.Getter
		errList       []error
	)
	...
	for devIdx, device := range vmCtx.VSphereVM.Spec.Network.Devices {
		for poolRefIdx, poolRef := range device.AddressesFromPools {
			...
			if deprecatedconditions.Has(ipAddrClaim, clusterv1beta1.ReadyCondition) {
	...
	if len(errList) > 0 {
		...
		deprecatedconditions.MarkFalse(vmCtx.VSphereVM,
	...
	if len(claims) == totalClaims {
		deprecatedconditions.SetAggregate(vmCtx.VSphereVM,
			infrav1.IPAddressClaimedCondition,
			claims,
			deprecatedconditions.AddSourceRef(),
			deprecatedconditions.WithStepCounter())

		if len(v1beta2Claims) > 0 {
			if err := deprecatedv1beta2conditions.SetAggregateCondition(v1beta2Claims, vmCtx.VSphereVM, clusterv1beta1.ReadyV1Beta2Condition, deprecatedv1beta2conditions.TargetConditionType(infrav1.VSphereVMIPAddressClaimsFulfilledV1Beta2Condition)); err != nil {
	...
	case totalClaims == claimsFulfilled:
		deprecatedconditions.MarkTrue(vmCtx.VSphereVM, infrav1.IPAddressClaimedCondition)
		deprecatedv1beta2conditions.Set(vmCtx.VSphereVM, metav1.Condition{
			Type:   infrav1.VSphereVMIPAddressClaimsFulfilledV1Beta2Condition,
			Status: metav1.ConditionTrue,
	...
	case claimsFulfilled < totalClaims && claimsCreated > 0:
		deprecatedconditions.MarkFalse(vmCtx.VSphereVM, infrav1.IPAddressClaimedCondition,
