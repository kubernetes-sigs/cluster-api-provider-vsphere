...
func Test_buildIPAMDeviceConfigs(t *testing.T) {
	var (
		...
		claim1, claim2, claim3       *ipamv1b1.IPAddressClaim
		address1, address2, address3 *ipamv1b1.IPAddress
		g                            *gomega.WithT
	)
	...
	before := func() {
		...
		claim1 = &ipamv1b1.IPAddressClaim{
		...
		claim2 = &ipamv1b1.IPAddressClaim{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "vsphereVM1-0-1",
				Namespace: namespace,
			},
		...
		claim3 = &ipamv1b1.IPAddressClaim{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "vsphereVM1-0-2",
				Namespace: namespace,
			},
		...
		address1 = &ipamv1b1.IPAddress{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "vsphereVM1-0-0-address0",
				Namespace: namespace,
			},
		}
		address2 = &ipamv1b1.IPAddress{
			ObjectMeta: metav1.ObjectMeta{
	...
	t.Run("when a device has a IPAddressPool", func(_ *testing.T) {
		...
		ipAddrClaim := &ipamv1b1.IPAddressClaim{}
...
func Test_BuildState(t *testing.T) {
	...
	t.Run("when one device has no pool and is DHCP true, and one device has a IPAddressPool", func(_ *testing.T) {
		...
		claim := &ipamv1b1.IPAddressClaim{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "vsphereVM1-1-0",
				Namespace: "my-namespace",
			},
		}
		address := &ipamv1b1.IPAddress{
			ObjectMeta: metav1.ObjectMeta{
	...
	t.Run("when realized IP addresses are incorrect", func(t *testing.T) {
		...
		beforeWithClaimsAndAddressCreated := func() {
			...
			claim1 = &ipamv1b1.IPAddressClaim{
				...
				Status: ipamv1b1.IPAddressClaimStatus{
