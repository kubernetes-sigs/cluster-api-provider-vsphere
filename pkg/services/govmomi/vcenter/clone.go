...
func Clone(ctx *context.VMContext, bootstrapData []byte) error {
	...
	if ctx.VSphereVM.Spec.DatastoreCluster != "" {
		...
		// if only the datastore cluster is mentioned
		if datastoreRef == nil {
			rs, _ := datastoreCluster.Children(ctx)
			rand.Seed(time.Now().UnixNano())
			datastoreRef = types.NewReference(rs[rand.Intn(len(rs))].Reference())
			spec.Location.Datastore = datastoreRef
		} else {
			// if the datastore is also defined by the user
			// verify that the datastore is a member of the datastoreCluster
			memberDatastoreFound := false
			memberDatastoreRefs, _ := datastoreCluster.Children(ctx)
			for _, datastore := range memberDatastoreRefs {
				datastoreName, err := object.NewDatastore(ctx.Session.Client.Client, datastore.Reference()).ObjectName(ctx)
				if err != nil {
					return errors.Wrapf(err, "unable to fetch datastore name for datastoreCluster %s", ctx.VSphereVM.Spec.DatastoreCluster)
				}
				if ctx.VSphereVM.Spec.Datastore == datastoreName {
					memberDatastoreFound = true
					spec.Location.Datastore = types.NewReference(datastore.Reference())
					break
				}
			}
			if !memberDatastoreFound {
				return errors.New(fmt.Sprintf("unable to find specified datastore: %s in member list of datastores for storageCluster", ctx.VSphereVM.Spec.Datastore))
			}
		}
	}
