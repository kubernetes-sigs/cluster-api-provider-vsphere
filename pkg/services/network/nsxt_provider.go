...
func (np *nsxtNetworkProvider) verifyNSXTVirtualNetworkStatus(vspherecluster *vmwarev1.VSphereCluster, vnet *ncpv1.VirtualNetwork) error {
	...
	for _, condition := range vnet.Status.Conditions {
		...
		if condition.Status != "True" {
			deprecatedconditions.MarkFalse(vspherecluster, vmwarev1.ClusterNetworkReadyCondition, vmwarev1.ClusterNetworkProvisionFailedReason, clusterv1beta1.ConditionSeverityWarning, condition.Message)
			deprecatedv1beta2conditions.Set(vspherecluster, metav1.Condition{
	...
	if !hasReadyCondition {
		deprecatedconditions.MarkFalse(vspherecluster, vmwarev1.ClusterNetworkReadyCondition, vmwarev1.ClusterNetworkProvisionFailedReason, clusterv1beta1.ConditionSeverityWarning, "No Ready status for virtual network")
		deprecatedv1beta2conditions.Set(vspherecluster, metav1.Condition{
			Type:    vmwarev1.VSphereClusterNetworkReadyV1Beta2Condition,
			Status:  metav1.ConditionFalse,
			Reason:  vmwarev1.VSphereClusterNetworkNotReadyV1Beta2Reason,
	...
	deprecatedconditions.MarkTrue(vspherecluster, vmwarev1.ClusterNetworkReadyCondition)
	deprecatedv1beta2conditions.Set(vspherecluster, metav1.Condition{
		Type:   vmwarev1.VSphereClusterNetworkReadyV1Beta2Condition,
...
func (np *nsxtNetworkProvider) ProvisionClusterNetwork(ctx context.Context, clusterCtx *vmware.ClusterContext) error {
	...
	if err != nil {
		deprecatedconditions.MarkFalse(clusterCtx.VSphereCluster, vmwarev1.ClusterNetworkReadyCondition, vmwarev1.ClusterNetworkProvisionFailedReason, clusterv1beta1.ConditionSeverityWarning, err.Error())
