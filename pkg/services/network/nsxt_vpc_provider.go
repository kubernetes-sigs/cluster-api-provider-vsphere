...
func (vp *nsxtVPCNetworkProvider) verifyNsxtVpcSubnetSetStatus(vspherecluster *vmwarev1.VSphereCluster, subnetset *nsxopv1.SubnetSet) error {
	...
	for _, condition := range subnetset.Status.Conditions {
		if condition.Type == nsxopv1.Ready && condition.Status != corev1.ConditionTrue {
...
func (vp *nsxtVPCNetworkProvider) ProvisionClusterNetwork(ctx context.Context, clusterCtx *vmware.ClusterContext) error {
	...
	log.Info("Provisioning ", "subnetset", networkName)
	defer log.Info("Finished provisioning", "subnetset", networkName)
...
// GetVMServiceAnnotations gets the subnet set name for a cluster.
// It then returns it as a key-value pair in a map.
// Returns an error if it can't retrieve the subnet set name.
func (vp *nsxtVPCNetworkProvider) GetVMServiceAnnotations(ctx context.Context, clusterCtx *vmware.ClusterContext) (map[string]string, error) {
	subnetsetName, err := vp.GetClusterNetworkName(ctx, clusterCtx)
	if err != nil {
		return nil, err
	}

	return map[string]string{NSXTVPCSubnetSetSelectorKey: subnetsetName}, nil
}
