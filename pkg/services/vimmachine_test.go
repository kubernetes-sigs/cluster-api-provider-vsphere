...
func Test_VimMachineService_ReconcileDelete(t *testing.T) {
	...
	t.Run("deletes VSphereVM", func(t *testing.T) {
		...
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(conditions.Get(machineCtx.VSphereMachine, infrav1.VMProvisionedCondition).Status).To(Equal(conditions.Get(vsphereVM, clusterv1beta1.ReadyCondition).Status))
	})
}
