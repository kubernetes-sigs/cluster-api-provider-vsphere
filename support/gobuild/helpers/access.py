...
def MakeAccessTarget(name, cls):
    class AccessTarget(cls):
        ...
        def GetCommands(self, hosttype):
            ...
            The list of commands is the list of commands to run the normal
            build, preceeded by a command to prepare the access build state
            and followed by commands to find accessed files in each source
