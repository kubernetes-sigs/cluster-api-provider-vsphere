...
// WaiterType indicates the type of check to use to determine if the
...
const (
	...
	ServiceWaiter WaiterType = "service"
...
func (c *Config) Validate() error {
	...
	for i, image := range c.Images {
		if image == "" {
			return errEmptyArg(fmt.Sprintf("Images[%d]", i))
		}
	}
...
func ComponentGeneratorForComponentSource(source ComponentSource) framework.ComponentGenerator {
