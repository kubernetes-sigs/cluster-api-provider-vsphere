...
func TestLoadConfig(t *testing.T) {
	g := gomega.NewWithT(t)
	config, err := frameworkx.LoadConfig([]byte(exampleConfigYAML1))
	g.Expect(err).ShouldNot(gomega.HaveOccurred())
	g.Expect(config).ShouldNot(gomega.BeNil())

	g.Expect(config.Images).To(gomega.HaveLen(1))
	g.Expect(config.Images[0]).To(gomega.Equal("capv-manager:e2e"))

	g.Expect(config.Components).To(gomega.HaveLen(5))

	g.Expect(config.Components[0].Waiters).To(gomega.HaveLen(2))
	g.Expect(config.Components[0].Waiters[1].Type).To(gomega.Equal(frameworkx.PodsWaiter))
}

const exampleConfigYAML1 = `---
images:
- capv-manager:e2e
