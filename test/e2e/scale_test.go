...
var _ = Describe("When testing the machinery for scale testing FP [vcsim] [supervisor] [ClusterClass]", func() {
...
func ScaleSpec(ctx context.Context, inputGetter func() ScaleSpecInput) {
	...
	It("Should create and delete workload clusters", func() {
		...
		flavor := "supervisor"
		if input.Flavor != nil {
			flavor = fmt.Sprintf("%s-supervisor", *input.Flavor)
		}
		...
		if input.E2EConfig.HasVariable("CLUSTER_CLASS_NAME") {
			testSpecificVariables["CLUSTER_CLASS_NAME"] = fmt.Sprintf("%s-supervisor", input.E2EConfig.GetVariable("CLUSTER_CLASS_NAME"))
		}

		// unset VSPHERE_ env variable (that in CI contains VMC data), so we are sure we use the value for vcsim
		for _, k := range []string{"VSPHERE_IMAGE_NAME", "VSPHERE_MACHINE_CLASS_NAME", "VSPHERE_PASSWORD", "VSPHERE_POWER_OFF_MODE", "VSPHERE_SERVER", "VSPHERE_SSH_AUTHORIZED_KEY", "VSPHERE_STORAGE_CLASS", "VSPHERE_STORAGE_POLICY", "VSPHERE_TLS_THUMBPRINT", "VSPHERE_USERNAME"} {
			Expect(os.Unsetenv(k)).To(Succeed())
		}

		// Set VSPHERE_ using defaults for vcsim.
		// Note: usually we read those variables from the ENV object, but this will be created when a cluster will be created, while we want to resolve some of those variable now in the baseWorkloadClusterTemplate.
		testSpecificVariables["VSPHERE_IMAGE_NAME"] = "ubuntu-2204-kube-v1.28.0"
		testSpecificVariables["VSPHERE_MACHINE_CLASS_NAME"] = "vcsim-default-vm-class"
		testSpecificVariables["VSPHERE_PASSWORD"] = "pass"
		testSpecificVariables["VSPHERE_POWER_OFF_MODE"] = "trySoft"
		testSpecificVariables["VSPHERE_SSH_AUTHORIZED_KEY"] = "ssh-rsa "
		testSpecificVariables["VSPHERE_STORAGE_CLASS"] = "vcsim-default-storage-class"
		testSpecificVariables["VSPHERE_STORAGE_POLICY"] = "vSAN Default Storage Policy"
		testSpecificVariables["VSPHERE_USERNAME"] = "user"

		// Set placeholders for variables that we are going to replace when a cluster will be created.
		testSpecificVariables["VSPHERE_SERVER"] = scaleClusterVSphereServerPlaceholder
		testSpecificVariables["VSPHERE_TLS_THUMBPRINT"] = scaleClusterVSphereTLSThumbprintPlaceholder
		testSpecificVariables["CONTROL_PLANE_ENDPOINT_IP"] = scaleClusterControlPlaneEndpointIPPlaceholder
		testSpecificVariables["CONTROL_PLANE_ENDPOINT_PORT"] = scaleClusterControlPlaneEndpointPortPlaceholder
		...
		testSpecificClusterctlConfigPath := fmt.Sprintf("%s-%s.yaml", strings.TrimSuffix(input.ClusterctlConfigPath, ".yaml"), specName)
		...
		// then setup the namespace for VMOperator & VCSim and deploy the ClusterClass in this namespace.
		if !input.DeployClusterInSeparateNamespaces {
			logf("Setup VMOperator and VCSim dependencies in namespace %s", namespace.Name)
			setupNamespaceWithVMOperatorDependenciesVCSim(input.BootstrapClusterProxy, namespace.Name)
	...
	for {
			...
			case clusterName, open := <-inputChan:
				...
				if postScaleClusterNamespaceCreated != nil {
					logf("Calling postScaleClusterNamespaceCreated for cluster %s in namespace %s", clusterName, namespaceName)
				...
				if deployClusterInSeparateNamespaces {
					logf("Apply ClusterClass in namespace %s", namespaceName)
				...
				// Adjust vcsim variables in Cluster YAML
				...
				// TODO: figure it out a way to cleanup addresses
...
func deleteClusterAndWaitWorker(ctx context.Context, inputChan <-chan string, resultChan chan<- workResult, wg *sync.WaitGroup, c crclient.Client, defaultNamespace string, deployClusterInSeparateNamespaces bool) {
	defer wg.Done()
...
func getClusterResourcesForUpgrade(ctx context.Context, c crclient.Client, namespace, clusterName string) clusterResources {
	cluster := &clusterv1.Cluster{}
	err := c.Get(ctx, crclient.ObjectKey{Namespace: namespace, Name: clusterName}, cluster)
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("error getting Cluster %s: %s", klog.KRef(namespace, clusterName), err))

	controlPlane := &controlplanev1.KubeadmControlPlane{}
	err = c.Get(ctx, crclient.ObjectKey{Namespace: namespace, Name: cluster.Spec.ControlPlaneRef.Name}, controlPlane)
